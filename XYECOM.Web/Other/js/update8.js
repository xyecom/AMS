...
Hongru.fader = function(){
 function init(anchor,options){
  ...
  this.a = options.auto?options.auto:1;
  this.cur = this.z = 0;
  this.l = li.length;
  this.img = [];
  ...
  nav_wp.id = 'fader-nav';
  nav_wp.style.cssText = 'position:absolute;right:0;bottom:0;padding:1px 0;';
  ...
  this.textH = nav_wp.offsetHeight;
    alt.style.cssText = 'color:#fff;font-size:12px;padding-left:10px;margin:0;position:absolute;left:0;bottom:0;overflow:hidden;width:100%;background:#000;opacity:0.3;filter:alpha(opacity=30);';
 ...
 init.prototype={
  ...
  pos:function(i){
   ...
   this.li[i].f = setInterval(new Function(this.anchor+'.fade('+i+')'),60);//图片闪烁速度
  },
  fade:function(i){
  var p=this.li[i];
   if(p.o>=100){
