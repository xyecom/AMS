function UploadImage(objName) {
    ...
    this._Init = function () {
        uploaddiv = $("#UploadFile")[0];
        ids = $("#Upload_IDs").val();
        files = $("#Upload_Files").val();

        this.maxAmount = parseInt($("#Upload_MaxAmount").val());
    ...
    this.DelItem = function (objIndex, id) {
        uploaddiv.removeChild($("#uploadFileItem" + objIndex)[0]);
        this.maxAmount++;
        if ("" != id) {//删除有文件的项目
            $("#Upload_DelIDs")[0].value = "" == $("#Upload_DelIDs").val() ? id : $("#Upload_DelIDs").val() + "," + id;
            this.AddItem("", "");
        }
    ...
    this.Show = function (id, objIndex) {
        ...
        $("#uploadbg").css("height", pageSize.docheight + "px");
        //$("#uploadbg").style.height = pageSize.docheight + "px";
        $("#uploadfrm").css("display", "block");
        ...
        $("#uploadfrm")[0].src = "/upload/UploadImage." + config.Suffix + "?TabName=" + $F("Upload_TabName") + "&iswatermark=" + $F("Upload_IsWaterMark") + "&AtID=" + id + "&tmp=" + Math.random();
        $("#uploadfrm")[0].style.width = dWidth + "px";
        $("#uploadfrm")[0].style.height = dHeight + "px";
        $("#uploadfrm")[0].style.left = x + 'px';
        $("#uploadfrm")[0].style.top = y + 'px';
    }
        ...
        if (filelist.length > 0) {
            if (0 == ac) {
                ...
                for (var ifile = 0; ifile < filelist.length; ifile++) {
                    $("#Upload_UpIDs")[0].value = "" == $F("Upload_UpIDs") ? filelist[ifile].id : $F("Upload_UpIDs") + "," + filelist[ifile].id;
                    this.AddItem(filelist[ifile].id, filelist[ifile].url);
                }
                this.AddItem("", "");
            }
            else {
                $("#Img" + editIndex)[0].innerHTML = this.GetImgHtml(filelist[0].url);
            }
        }
        if ("" != msg) sAlert(msg);
    }
    ...
    this.Close = function () {
        $("#uploadbg")[0].style.display = "none";
        $("#uploadfrm")[0].style.display = "none";
        $("#uploadfrm")[0].src = "";
    }
...
function UploadFileForFile(objName) {
    ...
    this.Uploading = function () {
        ...
        $("#uploadingbg")[0].style.display = "block";
        $("#uploadingbg")[0].style.height = (pageSize.height + scrollPos.scrollY) + "px";
        $("#uploading")[0].style.display = "block";

        var x = Math.round(pageSize.width / 2) - (dWidth / 2) + scrollPos.scrollX;
        var y = Math.round(pageSize.height / 2) - (dHeight / 2) + scrollPos.scrollY;

        $("#uploading")[0].style.width = dWidth + "px";
        $("#uploading")[0].style.height = dHeight + "px";
        $("#uploading")[0].style.left = x + 'px';
        $("#uploading")[0].style.top = y + 'px';
    }
    ...
    this.DelFile = function (objIndex) {
        uploadfilefrm.removeChild($("#uploadfileitemfrm" + objIndex)[0]);
        this.maxAmount++;
...
function UploadFile(objName) {
    ...
    this.DelItem = function (objIndex, id) {
        uploaddiv.removeChild($("#_uploadFileItem" + objIndex)[0]);
        this.maxAmount++;
        if ("" != id) {//删除有文件的项目
            $("#_Upload_DelIDs")[0].value = "" == $F("_Upload_DelIDs") ? id : $F("_Upload_DelIDs") + "," + id;
            this.AddItem("", "");
        }
        else {//删除新增文件 则清除新增文件的div位置索引
            addIndex = 0;
    ...
    this.Show = function (id, objIndex) {
        ...
        $("#_uploadbg")[0].style.display = "block";
        $("#_uploadbg")[0].style.height = pageSize.docheight + "px";
        $("#_uploadfrm")[0].style.display = "block";

        var x = Math.round(pageSize.width / 2) - (dWidth / 2) + scrollPos.scrollX;
        var y = Math.round(pageSize.height / 2) - (dHeight / 2) + scrollPos.scrollY;
        $("#_uploadfrm")[0].src = "/upload/UploadImage." + config.Suffix + "?filetype=file&TabName=" + $F("_Upload_TabName") + "&AtID=" + id + "&tmp=" + Math.random();
        $("#_uploadfrm")[0].style.width = dWidth + "px";
        $("#_uploadfrm")[0].style.height = dHeight + "px";
        $("#_uploadfrm")[0].style.left = x + 'px';
        $("#_uploadfrm")[0].style.top = y + 'px';
    }
    this.Save = function (msg, filelist) {
        if (filelist.length > 0) {
            if (0 == ac) {
                this.DelItem(addIndex, "");
                for (var ifile = 0; ifile < filelist.length; ifile++) {
                    $("#_Upload_UpIDs")[0].value = "" == $F("_Upload_UpIDs") ? filelist[ifile].id : $F("_Upload_UpIDs") + "," + filelist[ifile].id;
                    this.AddItem(filelist[ifile].id, filelist[ifile].url);
                }
                this.AddItem("", "");
            }
            else {
                $("#File" + editIndex)[0].innerHTML = filelist[0].url;
            }
        }
        if ("" != msg) sAlert(msg);
    }
...
function UploadFileForAspx(objName) {
    ...
    this.AddDeleteBtn = function (objindex) {
        if (!$("#_btnDel" + objindex)[0] && $("#_uploadfileitemfrm" + objindex)[0]) {
            var btndel = "<input type=\"button\" id=\"" + "_btnDel" + objindex + "\" onclick=\"" + obj + ".DelFile(" + objindex + ");\" />";
            insertHtml("beforeend", $("#_uploadfileitemfrm" + objindex)[0], btndel);
        }
    }
    this.LastFileIndex = function (objindex) {
        for (var i = this.maxAmount; i > objindex; i--) {
            if ($("#_uploadfileitemfrm" + i)[0])
                return i;
        }
        return 0;
    }
    this.Uploading = function () {
        ...
        $("#_uploadingbg")[0].style.display = "block";
        $("#_uploadingbg")[0].style.height = (pageSize.height + scrollPos.scrollY) + "px";
        $("#_uploading")[0].style.display = "block";

        var x = Math.round(pageSize.width / 2) - (dWidth / 2) + scrollPos.scrollX;
        var y = Math.round(pageSize.height / 2) - (dHeight / 2) + scrollPos.scrollY;

        $("#_uploading")[0].style.width = dWidth + "px";
        $("#_uploading")[0].style.height = dHeight + "px";
        $("#_uploading")[0].style.left = x + 'px';
        $("#_uploading")[0].style.top = y + 'px';
    }
    this.AddFileItem = function () {
        if (this.index > this.maxAmount) return;
        var uploadfileitem = document.createElement("div");
    ...
    this.DelFile = function (objIndex) {
        uploadfilefrm.removeChild($("#_uploadfileitemfrm" + objIndex)[0]);
        this.maxAmount++;
        this.AddFile(objIndex);
    }
}
