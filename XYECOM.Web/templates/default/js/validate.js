...
function SetbuySearchURL() {
    var key = $F("txtsearchkey").replace(/\s/g, "");
    //keyword=$1^jinji=$2^data=$3^pagesize=$4^style=$5^pageindex=$6
    if (!CheckSearchKey(key)) return false;
//    if (config.BogusStatic) {
//        var url = config.WebURL + "search/buy_search-" + key + "-----." + config.Suffix;
//    }
//    else {
        url = config.WebURL + "search/buy_search." + config.Suffix + "?keyword=" + key;
//    }
    location = url;
}
