<%@ Page language="c#" AutoEventWireup="false" EnableViewState="false" Inherits="XYECOM.Page.user.ordersalelist,XYECOM.Page" %>
...
System.Web.HttpContext.Current.Response.End();
	...
	XYBody.Append(" <ul>\r\n");
	XYBody.Append(" <li class=\"on\"><a href =\"");	XYBody.Append(config.WebURL);	XYBody.Append("user/ordersalelist.");	XYBody.Append(config.Suffix);	XYBody.Append("\">正在进行的交易</a> </li>\r\n");
	XYBody.Append(" <li class=\"out\"><a href =\"");	XYBody.Append(config.WebURL);	XYBody.Append("user/ordersalethrulist.");	XYBody.Append(config.Suffix);	XYBody.Append("\">已完成的交易</a></li> \r\n");
	XYBody.Append(" </ul>\r\n");
	XYBody.Append("</div>\r\n");
	XYBody.Append("<div class=\"ly_tit32\">\r\n");
	XYBody.Append("						&nbsp;开始时间： \r\n");
	XYBody.Append("				<select name=\"txtTime\">\r\n");
	XYBody.Append("							<option value=\"0\" selected=\"selected\" 	>全部时间段</option>\r\n");
	XYBody.Append("				<option value=\"3\" 	>最近3天</option>\r\n");
	XYBody.Append("				<option value=\"5\" 	>最近5天</option>\r\n");
	XYBody.Append("				<option value=\"15\" 	>最近15天</option>\r\n");
	XYBody.Append("				<option value=\"30\" 	 >最近一个月</option>\r\n");
	XYBody.Append("				<option value=\"60\" 	>最近两个月</option>\r\n");
	XYBody.Append("				<option value=\"90\" 	>最近三个月</option>\r\n");
	XYBody.Append("					</select> \r\n");
	XYBody.Append("                 &nbsp;交易状态：\r\n");
	XYBody.Append("				<select name=\"txtType\">\r\n");
	XYBody.Append("				<option value=\"0\" 	>所有交易</option>\r\n");
	XYBody.Append("				<option value=\"1\" 	>等待您补充运费</option>\r\n");
	XYBody.Append("				<option value=\"2\" 	>等待买家付款</option>\r\n");
	XYBody.Append("				<option value=\"3\" 	>等待您确认付款</option>\r\n");
	XYBody.Append("				<option value=\"4\" 	>等待您发货</option>\r\n");
	XYBody.Append("				<option value=\"5\" 	>等待买家确认收货</option>\r\n");
	XYBody.Append("				<option value=\"6\" 	>买家已确认收货</option>\r\n");
	XYBody.Append("				<option value=\"7\" 	>买家要求退货</option>\r\n");
	XYBody.Append("				<option value=\"8\" 	>您确认退货</option>\r\n");
	XYBody.Append("					</select> \r\n");
	XYBody.Append("						<input name=\"searchbutton\" class=\"button\" type=\"submit\" value=\"查 看\" />\r\n");
	XYBody.Append("</div>\r\n");
	XYBody.Append("</div>  \r\n");
	XYBody.Append("  <table  class=\"content\" width=\"100%\">\r\n");
	XYBody.Append("  <caption> 共有<em>(");	XYBody.Append(count.ToString());	XYBody.Append(")</em>符合条件！</caption>\r\n");
	XYBody.Append("  <thead>\r\n");
	XYBody.Append("	  <tr>\r\n");
	XYBody.Append("	    <th style=\"height: 20px\">序号</th>\r\n");
	XYBody.Append("	     <th style=\"height: 20px\">订单编号</th>\r\n");
	XYBody.Append("		  <th style=\"height: 20px\" >货品名称</th>\r\n");
	XYBody.Append("		  <th style=\"height: 20px\" >买家</th>\r\n");
	XYBody.Append("		  <th style=\"height: 20px\" >交易金额</th>\r\n");
	XYBody.Append("		  <th style=\"height: 20px\">开始时间</th>		  \r\n");
	XYBody.Append("		  <th style=\"height: 20px\" >交易状态</th>\r\n");
	XYBody.Append("           <th style=\"height: 20px\">操作</th>\r\n");
	XYBody.Append("           <th style=\"height: 20px\">查看</th>\r\n");
	XYBody.Append("	  </tr>\r\n");
	XYBody.Append("  </thead>\r\n");
	XYBody.Append("  <tbody>\r\n");
	...
	XYBody.Append("  <tr>  \r\n");
	XYBody.Append("    <td >");	XYBody.Append(sd__loop__id.ToString());	XYBody.Append("</td> \r\n");
	XYBody.Append("    <td>" + sd["OF_Code"].ToString().Trim() + "</td> \r\n");
	XYBody.Append("     <td><a href=\" \r\n");

	if (sd["SD_HTMLPage"].ToString().Trim()!="")
	{

	XYBody.Append("" + sd["SD_HTMLPage"].ToString().Trim() + "\r\n");
	...
	XYBody.Append("");	XYBody.Append(config.WebURL);	XYBody.Append("search/sell-offer-" + sd["SD_ID"].ToString().Trim() + ".");	XYBody.Append(config.Suffix);	XYBody.Append(" \r\n");

	}
	else
	{

	XYBody.Append("");	XYBody.Append(config.WebURL);	XYBody.Append("search/sell.");	XYBody.Append(config.Suffix);	XYBody.Append("?flag=offer&infoid=" + sd["SD_ID"].ToString().Trim() + "\r\n");
...
	XYBody.Append( Method.left(sd["SD_Title"].ToString().Trim(),10));
	XYBody.Append("</a></td>    \r\n");
	XYBody.Append("    <td>  <a href=\"\r\n");

	if (sd["U_HtmlPage1"].ToString().Trim()!="")
	{

	XYBody.Append("" + sd["U_HtmlPage1"].ToString().Trim() + "\r\n");
	...
	XYBody.Append(" http://" + sd["U_Name1"].ToString().Trim() + "domainhost/\r\n");

	}
	else
	{

	XYBody.Append(" ");	XYBody.Append(config.WebURL);	XYBody.Append("shop/" + sd["U_Name1"].ToString().Trim() + "/index.");	XYBody.Append(config.Suffix);	XYBody.Append("\r\n");
	...
	XYBody.Append("  \" target=\"_blank\" >\r\n");
	XYBody.Append( Method.left(sd["UI_Name"].ToString().Trim(),10));
	XYBody.Append("</a></td>\r\n");
	XYBody.Append("    <td class=\"orange\">" + sd["OF_Money"].ToString().Trim() + "</td>\r\n");
	XYBody.Append("    <td>" + sd["OF_BeginTime"].ToString().Trim() + "</td>\r\n");
	XYBody.Append("    <td >\r\n");

	if (sd["OF_Type"].ToString().Trim()=="1")
	{

	XYBody.Append("    等待您补充运费\r\n");

	}	//end if


	if (sd["OF_Type"].ToString().Trim()=="2")
	{

	XYBody.Append("    等待买家确认运费并付款\r\n");

	}	//end if


	if (sd["OF_Type"].ToString().Trim()=="3")
	{

	XYBody.Append("    等待您确认收款\r\n");

	}	//end if


	if (sd["OF_Type"].ToString().Trim()=="4")
	{

	XYBody.Append("    等待您确认发货\r\n");

	}	//end if


	if (sd["OF_Type"].ToString().Trim()=="5")
	{

	XYBody.Append("    等待买家确认收货\r\n");

	}	//end if


	if (sd["OF_Type"].ToString().Trim()=="6")
	{

	XYBody.Append("    买家已确认收货\r\n");

	}	//end if


	if (sd["OF_Type"].ToString().Trim()=="7")
	{

	XYBody.Append("    买家要求退货\r\n");

	}	//end if


	if (sd["OF_Type"].ToString().Trim()=="8")
	{

	XYBody.Append("    您已经同意退货\r\n");
	...
	XYBody.Append("   <a href=\"");	XYBody.Append(config.WebURL);	XYBody.Append("user/ordersaleinfo.");	XYBody.Append(config.Suffix);	XYBody.Append("?OF_ID=" + sd["OF_ID"].ToString().Trim() + "\" title=\"点击补充运费\">补充运费</a>\r\n");
	...
	XYBody.Append("    <a href=\"");	XYBody.Append(config.WebURL);	XYBody.Append("user/ordersaleinfo.");	XYBody.Append(config.Suffix);	XYBody.Append("?OF_ID=" + sd["OF_ID"].ToString().Trim() + "\"title=\"点击修改运费\">修改运费</a>\r\n");
	...
	XYBody.Append("    <a href=\"#\"onclick=\"onupdateof(" + sd["OF_ID"].ToString().Trim() + ",3)\" title=\"点击确认收款\">确认收款</a>\r\n");
	...
	XYBody.Append("   <a href=\"#\"onclick=\"onupdateof(" + sd["OF_ID"].ToString().Trim() + ",4)\" title=\"点击确认发货\" >确认发货</a>\r\n");
	...
	XYBody.Append("   <a href=\"#\" onclick=\"onupdateof(" + sd["OF_ID"].ToString().Trim() + ",8)\" title=\"点击同意退货\" >同意退货</a>/\r\n");
	XYBody.Append("   <a href=\"#\"onclick=\"onupdateof(" + sd["OF_ID"].ToString().Trim() + ",12)\" title=\"点击拒绝退货\">拒绝退货</a>\r\n");
	...
	XYBody.Append("   <a href=\"#\"onclick=\"onupdateof(" + sd["OF_ID"].ToString().Trim() + ",9)\" title=\"点击退货成功\">退货成功</a>\r\n");

	}	//end if

	XYBody.Append("</td> <td>\r\n");
	XYBody.Append("    <a href=\"");	XYBody.Append(config.WebURL);	XYBody.Append("user/orderlist.");	XYBody.Append(config.Suffix);	XYBody.Append("?OF_ID=" + sd["OF_ID"].ToString().Trim() + "\"> 查看</a>\r\n");
	XYBody.Append("    </td>\r\n");
	XYBody.Append("    </tr>    \r\n");
	...
	XYBody.Append("<div class=\"ly_tx\">\r\n");
	XYBody.Append(" <h4>请注意</h4>\r\n");
	XYBody.Append(" <ul>\r\n");
	XYBody.Append("  <li>· 产品订购后，请及时处理，\"30\"天未处理则转为过期订单。</li>\r\n");
	XYBody.Append(" <li>· 产品订购后订单不能删除 。</li>\r\n");
	XYBody.Append(" <li>· 在交易过程中，卖家确认添加运费和优惠后产品数量、送货地址等不能再次修改。</li>\r\n");
	XYBody.Append(" <li>· 在交易过程中，买家确认付款后请卖家及时确认收款并发货。</li>\r\n");
	XYBody.Append(" </ul>\r\n");
	XYBody.Append("</div>\r\n");
	XYBody.Append("</form>\r\n");
	XYBody.Append("</div>\r\n");
