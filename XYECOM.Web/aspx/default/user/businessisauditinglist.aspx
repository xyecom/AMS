<%@ Page language="c#" AutoEventWireup="false" EnableViewState="false" Inherits="XYECOM.Page.user.businessisauditinglist,XYECOM.Page" %>
...
System.Web.HttpContext.Current.Response.End();
	...
	int eibs__loop__id=0;
	foreach(DataRow eibs in infolist.Rows)
	{
		eibs__loop__id++;

	XYBody.Append("          <tr>\r\n");
	XYBody.Append("            <td ><input  name=\"sd_id\" id=\"sd_id\" type=\"checkbox\" value=\"" + eibs["IBI_ID"].ToString().Trim() + "\" /></td>\r\n");
	XYBody.Append("            <td >\r\n");
	 infoimg = GetInfoImgHref(eibs["IBI_ID"].ToString());
	

	if (infoimg=="")
	{

	XYBody.Append("                    <img src=\"/common/images/DefaultImg.gif\" alt=\"" + eibs["IBI_Title"].ToString().Trim() + "\" width=\"60\" height=\"60\" />\r\n");

	}
	else
	{

	XYBody.Append("                    <img src=\"");	XYBody.Append(infoimg.ToString());	XYBody.Append("\" alt=\"" + eibs["IBI_Title"].ToString().Trim() + "\" width=\"60\" height=\"60\" />\r\n");

	}	//end if

	XYBody.Append("            </td>\r\n");
	XYBody.Append("            <td class=\"infoTitle\">\r\n");
	XYBody.Append( Method.left(eibs["IBI_Title"].ToString().Trim(),20));
	XYBody.Append("</td>\r\n");
	XYBody.Append("            <td class=\"wbg_setTime\">" + eibs["IBI_PublishDate"].ToString().Trim() + "</td>  \r\n");
	XYBody.Append("            <td style=\"text-align:center;\">\r\n");

	if (eibs["IBI_Pause"].ToString().Trim()=="False")
	{
	...
	XYBody.Append("</td>\r\n");
	XYBody.Append("            <td class=\"wbg_clicknum\">" + eibs["IBI_ClickNum"].ToString().Trim() + "</td>\r\n");
	XYBody.Append("            <td ><a href=\"");	XYBody.Append(config.WebURL);	XYBody.Append("user/receivemessagelist.");	XYBody.Append(config.Suffix);	XYBody.Append("?type='investment'&InfoID=" + eibs["IBI_ID"].ToString().Trim() + "\"><em class=\"wbg_messageNum\">");	XYBody.Append(summessage.ToString());	XYBody.Append("</em></a> 条&nbsp;</td>\r\n");
	XYBody.Append("           <td><input type =\"button\" class =\"button\"  value =\"修改\" onclick =\"location.href='");	XYBody.Append(config.WebURL);	XYBody.Append("user/adv_post.");	XYBody.Append(config.Suffix);	XYBody.Append("?ename=");	XYBody.Append(pageinfo.ModuleFlag);	XYBody.Append("&flag=" + eibs["IBI_Sign"].ToString().Trim() + "&info_ID=" + eibs["IBI_ID"].ToString().Trim() + "'\"/></td>\r\n");
	XYBody.Append("            </tr>\r\n");
