<%@ Page language="c#" AutoEventWireup="false" EnableViewState="false" Inherits="XYECOM.Page.user.brandlist,XYECOM.Page" %>
...
System.Web.HttpContext.Current.Response.End();
	...
	XYBody.Append("      <h2><span><input type=\"button\" class=\"button\" value=\"发布新的品牌信息\"  onclick =\"window.location.href='");	XYBody.Append(config.WebURL);	XYBody.Append("user/brandadd.");	XYBody.Append(config.Suffix);	XYBody.Append("'\"/></span>管理品牌信息</h2>\r\n");
	...
	XYBody.Append("		      <td>品牌名称</td>\r\n");
	XYBody.Append("		      <td> 类别</td>\r\n");
	...
	int eb__loop__id=0;
	foreach(DataRow eb in infolist.Rows)
	{
		eb__loop__id++;

	XYBody.Append("      <tr>\r\n");
	XYBody.Append("        <td class=\"w_25\"><input id=\"sd_id\" name=\"sd_id\" type=\"checkbox\" value=\"" + eb["B_ID"].ToString().Trim() + "\" /></td>   \r\n");
	XYBody.Append("        <td class=\"infoTitle\">\r\n");

	if (eb["B_HtmlPage"].ToString().Trim()!="")
	{

	XYBody.Append("            <a href=\"");	XYBody.Append(config.WebURL);	XYBody.Append("" + eb["B_HtmlPage"].ToString().Trim() + "\">" + eb["B_Type"].ToString().Trim() + "</a>\r\n");
	...
	XYBody.Append("                <a href=\"/brand/info-" + eb["B_ID"].ToString().Trim() + ".");	XYBody.Append(config.Suffix);	XYBody.Append("\" target=\"_blank\">" + eb["B_Type"].ToString().Trim() + "</a>\r\n");

	}
	else
	{

	XYBody.Append("            <a href=\"/brand/info.");	XYBody.Append(config.Suffix);	XYBody.Append("?infoid=" + eb["B_ID"].ToString().Trim() + "\" target=\"_blank\">" + eb["B_Type"].ToString().Trim() + "</a>\r\n");
	...
	XYBody.Append("        </td>\r\n");
	XYBody.Append("        <td>" + eb["PT_Name"].ToString().Trim() + "</td>\r\n");
	XYBody.Append("        <td class=\"wbg_setTime\">" + eb["addtime"].ToString().Trim() + "</td>\r\n");
	XYBody.Append("        <td class=\"wbg_clicknum\" style=\"text-align:center;\">" + eb["B_ClickNum"].ToString().Trim() + "</td>\r\n");
	XYBody.Append("        <td ><a href=\"");	XYBody.Append(config.WebURL);	XYBody.Append("user/receivemessagelist.");	XYBody.Append(config.Suffix);	XYBody.Append("?type='brand'&InfoID=" + eb["B_ID"].ToString().Trim() + "\"><em class=\"wbg_messageNum\">");	XYBody.Append(summessage.ToString());	XYBody.Append("</em></a> 条&nbsp;</td>\r\n");
	XYBody.Append("            <td><input type =\"button\" class =\"button\"  value =\"修改\" onclick =\"location.href='");	XYBody.Append(config.WebURL);	XYBody.Append("user/brandupdate.");	XYBody.Append(config.Suffix);	XYBody.Append("?B_ID=" + eb["B_ID"].ToString().Trim() + "';\"/>\r\n");
	XYBody.Append("       </td>\r\n");
	XYBody.Append("      </tr>\r\n");
	...
	XYBody.Append("                <input name=\"barnd\"  class=\"button\" type=\"submit\" value=\"删除\"  onclick=\"return bntDelete()\"/>\r\n");
	XYBody.Append("            </td>\r\n");
	XYBody.Append("        </tr>\r\n");
	XYBody.Append("    </table>\r\n");
