<%@ Page language="c#" AutoEventWireup="false" EnableViewState="false" Inherits="XYECOM.Page.user.surrogatenotauditinglist,XYECOM.Page" %>
...
System.Web.HttpContext.Current.Response.End();
	...
	int SY__loop__id=0;
	foreach(DataRow SY in infolist.Rows)
	{
		SY__loop__id++;

	XYBody.Append("   <tr> \r\n");
	XYBody.Append("    <td><input name=\"sd_id\" id=\"sd_id\" type=\"checkbox\" value=\"" + SY["S_ID"].ToString().Trim() + "\" /></td>\r\n");
	XYBody.Append("     <td >\r\n");
	 infoimg = GetInfoImgHref(SY["S_ID"].ToString());
	

	if (infoimg=="")
	{

	XYBody.Append("            <img src=\"/common/images/DefaultImg.gif\" alt=\"" + SY["S_Title"].ToString().Trim() + "\" width=\"60\" height=\"60\" />\r\n");

	}
	else
	{

	XYBody.Append("            <img src=\"");	XYBody.Append(infoimg.ToString());	XYBody.Append("\" alt=\"" + SY["S_Title"].ToString().Trim() + "\" width=\"60\" height=\"60\" />\r\n");

	}	//end if

	XYBody.Append("     </td>\r\n");
	XYBody.Append("     <td class=\"infoTitle\">\r\n");
	XYBody.Append( Method.left(SY["S_Title"].ToString().Trim(),20));
	XYBody.Append("</td>\r\n");
	XYBody.Append("    <td class=\"wbg_setTime\" >" + SY["S_AddTime"].ToString().Trim() + "</td>\r\n");
	XYBody.Append("    <td style=\"text-align:center;\">\r\n");

	if (SY["S_Pause"].ToString().Trim()=="True")
	{
	...
	XYBody.Append("    </td>\r\n");
	XYBody.Append("    <td class=\"wbg_clicknum\">" + SY["S_ClickNum"].ToString().Trim() + "</td>\r\n");
	XYBody.Append("    <td><a href=\"");	XYBody.Append(config.WebURL);	XYBody.Append("user/receivemessagelist.");	XYBody.Append(config.Suffix);	XYBody.Append("?type='service'&InfoID=" + SY["S_ID"].ToString().Trim() + "\"><em class=\"wbg_messageNum\">" + SY["S_MessageNum"].ToString().Trim() + "</em></a> 条&nbsp;</td>\r\n");
	XYBody.Append("    <td><input type =\"button\" class =\"button\"  value =\"修改\" onclick =\"location.href='");	XYBody.Append(config.WebURL);	XYBody.Append("user/adv_post.");	XYBody.Append(config.Suffix);	XYBody.Append("?ename=");	XYBody.Append(pageinfo.ModuleFlag);	XYBody.Append("&flag=" + SY["S_Flag"].ToString().Trim() + "&info_ID=" + SY["S_ID"].ToString().Trim() + "'\" /></td>\r\n");
	XYBody.Append("  </tr>\r\n");
