<%@ Page language="c#" AutoEventWireup="false" EnableViewState="false" Inherits="XYECOM.Page.user.engagelist,XYECOM.Page" %>
...
System.Web.HttpContext.Current.Response.End();
	...
	XYBody.Append("  <h3>招聘信息管理</h3>\r\n");
	XYBody.Append("     <form action=\"#\" id=\"\" method=\"post\">\r\n");
	XYBody.Append("     <table  class=\"content\">\r\n");
	XYBody.Append("  <thead>\r\n");
	XYBody.Append("	  <tr>\r\n");
	XYBody.Append("	  	  <th width=\"8%\">选择</th>\r\n");
	XYBody.Append("	      <th width=\"10%\">招聘部门</th>\r\n");
	XYBody.Append("		  <th width=\"17%\">招聘职位</th>\r\n");
	XYBody.Append("		   <th width=\"10%\">招聘状态</th>\r\n");
	XYBody.Append("		  <th width=\"10%\">浏览次数</th>\r\n");
	XYBody.Append("		   <th width=\"12%\">审核状态</th>\r\n");
	XYBody.Append("		  <th width=\"14%\">发布日期</th>\r\n");
	XYBody.Append("		  <th width=\"8%\">修改</th>\r\n");
	...
	int ei__loop__id=0;
	foreach(DataRow ei in infolist.Rows)
	{
		ei__loop__id++;

	XYBody.Append("  <tr>\r\n");
	XYBody.Append("  <td><input id=\"sd_id\" name=\"sd_id\" type=\"checkbox\" value=\"" + ei["EI_ID"].ToString().Trim() + "\" /></td>\r\n");
	XYBody.Append("   <td>" + ei["EI_Branch"].ToString().Trim() + "</td>\r\n");
	XYBody.Append("   <td>" + ei["EI_Job"].ToString().Trim() + "</td>  \r\n");
	XYBody.Append("   <td  class=\"orange\">\r\n");

	if (ei["EI_Puach"].ToString().Trim()=="False")
	{
	...
	XYBody.Append("   </td>\r\n");
	XYBody.Append("   <td>" + ei["EI_ClickNum"].ToString().Trim() + "次</td>\r\n");
	XYBody.Append("   <td class=\"orange\">\r\n");

	if (ei["AuditingState"].ToString().Trim()=="")
	{
	...
	if (ei["AuditingState"].ToString().Trim()=="0")
	{
	...
	if (ei["AuditingState"].ToString().Trim()=="1")
	{
	...
	XYBody.Append("    </td>\r\n");
	XYBody.Append("   <td>\r\n");
	XYBody.Append( Method.formatdatetime(ei["EI_AddDate"].ToString().Trim(),"yyyy-MM-dd"));
	XYBody.Append("</td>\r\n");
	XYBody.Append("   <td><a href=\"");	XYBody.Append(config.WebURL);	XYBody.Append("user/engageedit.");	XYBody.Append(config.Suffix);	XYBody.Append("?ei_id=" + ei["EI_ID"].ToString().Trim() + "\">修改</a></td>\r\n");
	XYBody.Append("   <td><a href=\"");	XYBody.Append(config.WebURL);	XYBody.Append("user/engageinfo.");	XYBody.Append(config.Suffix);	XYBody.Append("?ei_id=" + ei["EI_ID"].ToString().Trim() + "\">查看</a>/<a href=\"");	XYBody.Append(config.WebURL);	XYBody.Append("user/resumelist.");	XYBody.Append(config.Suffix);	XYBody.Append("?ei_id=" + ei["EI_ID"].ToString().Trim() + "\">简历</a></td>\r\n");
	XYBody.Append("    </tr>\r\n");
	XYBody.Append("  <input type=\"hidden\" id=\"audting\" name=\"audting\" value=\"" + ei["AuditingState"].ToString().Trim() + "\" />\r\n");
	...
	XYBody.Append("<tr>\r\n");
	XYBody.Append("<td><input id=\"chkAll\" name=\"chkAll\" type=\"checkbox\" value=\"\" onclick=\"CheckedAll();\"/>全选 \r\n");
	XYBody.Append("<input name=\"engage\"  type=\"submit\" class=\"button\" value=\"启用\" onclick =\"return resh(3);\" />\r\n");
	XYBody.Append("<input name=\"engage\"  type=\"submit\" class=\"button\" value=\"暂停\"  onclick =\"return resh(2);\"/>\r\n");
	XYBody.Append("<input name=\"engage\"  class=\"button\" type=\"submit\" value=\"删除\" onclick=\"return bntDelete();\"/>\r\n");
	XYBody.Append("<input name=\"refurbish\"  class=\"button\" type=\"submit\" value=\"刷新\" onclick=\"return resh(1);\"/>\r\n");
	XYBody.Append("<input name=\"engage\"  class=\"button\" type=\"button\" value=\"发布\" onclick=\"window.location.href='");	XYBody.Append(config.WebURL);	XYBody.Append("user/engageadd.");	XYBody.Append(config.Suffix);	XYBody.Append("';\"/>\r\n");
	XYBody.Append("</td>\r\n");
	XYBody.Append("<td>\r\n");
	XYBody.Append("</td>\r\n");
	XYBody.Append("</tr>\r\n");
