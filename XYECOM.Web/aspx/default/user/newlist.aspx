<%@ Page language="c#" AutoEventWireup="false" EnableViewState="false" Inherits="XYECOM.Page.user.newlist,XYECOM.Page" %>
...
System.Web.HttpContext.Current.Response.End();
	...
	XYBody.Append("     <h2>\r\n");
	XYBody.Append("        <span>\r\n");
	XYBody.Append("            <!--<input name=\"but02\" type=\"button\" value=\"投稿\" onclick =\"window.open('");	XYBody.Append(config.WebURL);	XYBody.Append("contributor.");	XYBody.Append(config.Suffix);	XYBody.Append("');\"  class =\"button\"/>&nbsp;-->\r\n");
	XYBody.Append("            <input name=\"new\"  class=\"button\" type=\"button\" value=\"发布资讯\" onclick=\"window.location.href='");	XYBody.Append(config.WebURL);	XYBody.Append("user/addnews.");	XYBody.Append(config.Suffix);	XYBody.Append("'\"/>\r\n");
	XYBody.Append("        </span>\r\n");
	XYBody.Append("        资讯管理\r\n");
	XYBody.Append("     </h2>\r\n");
	...
	XYBody.Append("            <ul>\r\n");
	XYBody.Append("                <li class=\"on\"><a href =\"");	XYBody.Append(config.WebURL);	XYBody.Append("user/newlist.");	XYBody.Append(config.Suffix);	XYBody.Append("\">资讯</a> </li>\r\n");
	XYBody.Append("                <!--<li class=\"out\"><a href =\"");	XYBody.Append(config.WebURL);	XYBody.Append("user/contributenewslist.");	XYBody.Append(config.Suffix);	XYBody.Append("\"> 投稿资讯</a></li>-->\r\n");
	XYBody.Append("            </ul>\r\n");
	XYBody.Append("        </div>\r\n");
	XYBody.Append("        <div class=\"ly_tit32\">\r\n");
	XYBody.Append("            标题关键字：<input type=\"text\"  id=\"key\" name =\"key\" value=\"");	XYBody.Append(key.ToString());	XYBody.Append("\"/>\r\n");
	XYBody.Append("            <input type=\"submit\" name=\"submit_search\" value=\"查询\" class =\"button\"/>\r\n");
	XYBody.Append("        </div>\r\n");
	XYBody.Append("          <thead>\r\n");
	XYBody.Append("	          <tr>\r\n");
	XYBody.Append("	          <th width=\"8%\">选择</th>\r\n");
	XYBody.Append("	          <th width=\"20%\">标 题</th>	  \r\n");
	XYBody.Append("	          <th width=\"20%\">发布时间</th>\r\n");
	XYBody.Append("	          <th width=\"17%\">审核状态</th>\r\n");
	XYBody.Append("               <th width=\"30%\">状态</th> \r\n");
	XYBody.Append("	          <th width=\"10%\">修改</th> \r\n");
	XYBody.Append("	          </tr>\r\n");
	XYBody.Append("          </thead>\r\n");
	XYBody.Append("    <tbody>\r\n");

	int n__loop__id=0;
	foreach(DataRow n in infolist.Rows)
	{
		n__loop__id++;

	XYBody.Append("      <tr> \r\n");
	XYBody.Append("      <td><input id=\"sd_id\" name=\"sd_id\" type=\"checkbox\" value=\"" + n["N_ID"].ToString().Trim() + "\" /></td>\r\n");
	XYBody.Append("       <td>\r\n");
	XYBody.Append( Method.left(n["N_title"].ToString().Trim(),40));
	XYBody.Append("</td>\r\n");
	XYBody.Append("       <td>" + n["N_addtime"].ToString().Trim() + "</td>\r\n");

	if (n["AuditingState"].ToString().Trim()=="1")
	{

	XYBody.Append("            <td>通过审核</td>\r\n");

	}
	else
	{

	XYBody.Append("            <td>审核未通过</td>\r\n");

	}	//end if


	if (n["State"].ToString().Trim()=="1")
	{

	XYBody.Append("        <td>\r\n");
	XYBody.Append("        未推荐 未置顶\r\n");
	XYBody.Append("        </td>\r\n");

	}	//end if


	if (n["State"].ToString().Trim()=="2")
	{

	XYBody.Append("        <td>\r\n");
	XYBody.Append("        已推荐 未置顶\r\n");
	XYBody.Append("        </td>\r\n");

	}	//end if


	if (n["State"].ToString().Trim()=="3")
	{

	XYBody.Append("        <td>\r\n");
	XYBody.Append("        未推荐 已置顶\r\n");
	XYBody.Append("        </td>\r\n");

	}	//end if


	if (n["State"].ToString().Trim()=="6")
	{

	XYBody.Append("        <td>\r\n");
	XYBody.Append("        已推荐 已置顶\r\n");
	XYBody.Append("        </td>\r\n");

	}	//end if

	XYBody.Append("         <td>\r\n");
	XYBody.Append("        <a href=\"");	XYBody.Append(config.WebURL);	XYBody.Append("user/editnew.");	XYBody.Append(config.Suffix);	XYBody.Append("?N_ID=" + n["N_ID"].ToString().Trim() + "&isflag=0\">修改</a>\r\n");
	XYBody.Append("        </td>\r\n");
	XYBody.Append("      </tr>\r\n");
	...
	XYBody.Append("<input name=\"new\"  class=\"button\" type=\"submit\" value=\"删除\"  onclick=\"return bntDelete()\"/>\r\n");
	XYBody.Append("<input name=\"commend\"  class=\"button\" type=\"submit\" value=\"推荐\"  onclick=\"return IsSelectInfo()\"/>\r\n");
	XYBody.Append("<input name=\"top\"  class=\"button\" type=\"submit\" value=\"置顶\"  onclick=\"return IsSelectInfo()\"/>\r\n");
	XYBody.Append("<input name=\"uncommend\"  class=\"button\" type=\"submit\" value=\"取消推荐\"  onclick=\"return IsSelectInfo()\"/>\r\n");
	XYBody.Append("<input name=\"untop\"  class=\"button\" type=\"submit\" value=\"取消置顶\"  onclick=\"return IsSelectInfo()\"/>\r\n");
	XYBody.Append("</td>\r\n");
	XYBody.Append("</tr>\r\n");
	XYBody.Append("</table>\r\n");
